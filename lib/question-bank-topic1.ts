...
export const topic1Questions: Question[] = [
  // Question 1 - Answer: C
  {
    ...
    option_a: 'A single batched API call with all files pre-loaded into context to minimize request overhead',
    option_b: 'Multiple isolated API calls with manual coordination logic between each analysis step',
    option_c: 'An agentic loop that allows Claude to iteratively use tools and refine reasoning until task completion',
    option_d: 'A simplified streaming workflow that processes all operations in one continuous request cycle',
    correct_answer: 'C',
  ...
  // Question 2 - Answer: B
  {
    ...
    option_a: 'end_turn indicating Claude needs additional reasoning cycles to proceed',
    option_b: 'tool_use indicating Claude has requested tool execution to continue',
    option_c: 'max_tokens indicating Claude needs extended context to complete the workflow',
    option_d: 'continue_iteration signaling Claude requires another processing cycle',
    correct_answer: 'B',
  ...
  // Question 3 - Answer: A
  {
    ...
    option_a: 'end_turn signaling Claude completed its response without requiring additional tools',
    option_b: 'completion_success indicating the workflow finished all pending tool operations',
    option_c: 'task_complete confirming Claude resolved all objectives without further action',
    option_d: 'iteration_end showing Claude reached a natural stopping point in processing',
    correct_answer: 'A',
  ...
  // Question 4 - Answer: D
  {
    ...
    option_a: 'All pending tool executions completed successfully but output formatting was truncated',
    option_b: 'The workflow reached its maximum iteration limit and terminated gracefully',
    option_c: 'Claude completed the task but the response buffer exceeded capacity constraints',
  ...
  // Question 5 - Answer: C
  {
    ...
    option_a: 'result_id field which uniquely identifies each tool execution result',
    option_b: 'execution_id field which tracks the sequential order of tool calls',
    option_c: 'tool_use_id field which matches between tool_use and tool_result blocks',
    option_d: 'correlation_token field which links related tool operations together',
    correct_answer: 'C',
  ...
  // Question 6 - Answer: B
  {
    ...
    option_a: 'Claude will intelligently skip missing tools and continue with available results',
    option_b: 'The API will return a validation error rejecting the incomplete request',
    option_c: 'The loop continues normally by treating missing results as null values',
    option_d: 'The system automatically retries execution for any missing tool results',
    correct_answer: 'B',
  ...
  // Question 7 - Answer: A
  {
    ...
    option_a: 'pause_turn indicating Claude wants explicit user confirmation before proceeding',
    option_b: 'await_approval signaling Claude needs authorization before executing sensitive actions',
    option_c: 'user_input_required showing Claude needs additional parameters before continuing',
    option_d: 'approval_pending indicating Claude detected a transaction requiring user consent',
    correct_answer: 'A',
  ...
  // Question 8 - Answer: D
  {
    ...
    option_a: 'A critical tool execution failed and Claude cannot proceed with the workflow',
    option_b: 'The response exceeded token limits and was automatically truncated by the system',
    option_c: 'The agentic loop detected an error condition and should retry the operation',
    option_d: 'Claude declined to respond due to content safety policies or guidelines',
    correct_answer: 'D',
  ...
  // Question 9 - Answer: C
  {
    ...
    option_a: 'Implement automatic circuit breakers that pause execution when anomalies are detected',
    option_b: 'Configure max_tokens with exponential backoff to gradually reduce response sizes',
    option_c: 'Implement both max_turns and max_budget_usd for comprehensive cost and iteration control',
    option_d: 'Set max_iterations with automatic throttling when approaching threshold limits',
    correct_answer: 'C',
  ...
  // Question 10 - Answer: B
  {
    ...
    option_a: 'No, this indicates an API error - responses should contain only one tool request',
    option_b: 'Yes, Claude can request multiple tool executions in a single response',
    option_c: 'This only occurs when batch processing mode is enabled in your configuration',
    option_d: 'Multiple tool requests require the parallel_execution flag to be set',
    correct_answer: 'B',
  ...
  // Question 11 - Answer: A
  {
    ...
    option_a: 'user, assistant, user, assistant in strict alternating sequence',
    option_b: 'assistant, user, assistant, user allowing AI-initiated conversations',
    option_c: 'user, user, assistant, assistant enabling batched message grouping',
    option_d: 'Any sequence order as long as role metadata is properly specified',
    correct_answer: 'A',
  ...
  // Question 12 - Answer: D
  {
    ...
    option_a: 'Replace the previous assistant message by embedding results into the original request',
    option_b: 'Merge the results directly into the existing tool_use message content',
    option_c: 'Update the system prompt to include results for the next iteration',
  ...
  // Question 13 - Answer: C
  {
    ...
    option_a: 'Server-side tools execute with lower latency due to optimized infrastructure',
    option_b: 'Client-side tools provide better caching and reduced per-request API costs',
    option_c: 'Client-side tools are defined and executed by you, server-side tools run on Anthropic\'s infrastructure',
    option_d: 'Server-side tools support parallel execution while client-side tools run sequentially',
    correct_answer: 'C',
  ...
  // Question 14 - Answer: B
  {
    ...
    option_a: '10-20% improvement in overall task completion and accuracy metrics',
    option_b: '50%+ improvement in complex task success rates compared to single-turn',
    option_c: '5-15% improvement with diminishing returns for longer workflows',
    option_d: '25-35% improvement primarily in reasoning-heavy tasks and analysis',
    correct_answer: 'B',
  ...
  // Question 15 - Answer: A
  {
    ...
    option_b: 'This indicates a formatting error - responses should contain only tool_use blocks',
    option_c: 'This suggests a configuration issue - text and tools should be in separate responses',
    option_d: 'This only occurs when verbose mode or extended_thinking is enabled',
    correct_answer: 'A',
  ...
  // Question 16 - Answer: D
  {
    ...
    option_a: 'Response streaming configuration for better performance and cost monitoring',
    option_b: 'Input validation and sanitization which would have prevented malformed requests',
    option_c: 'Circuit breaker pattern with automatic failure detection and recovery',
    option_d: 'max_turns limit to prevent infinite loops and excessive API costs',
    correct_answer: 'D',
  ...
  // Question 17 - Answer: C
  {
    ...
    option_a: 'Context size remains relatively constant through automatic compression mechanisms',
    option_b: 'Only tool execution results accumulate while request messages are automatically pruned',
    option_c: 'Each iteration accumulates messages, tool_use blocks, and tool_result blocks, growing the context',
    option_d: 'The context window automatically shrinks older messages using LRU eviction',
    correct_answer: 'C',
  ...
  // Question 18 - Answer: B
  {
    ...
    option_a: 'Enable dynamic context expansion by upgrading to models with larger windows',
    option_b: 'Periodically summarize earlier turns and prune conversation history strategically',
    option_c: 'Implement conversation checkpointing and restart from saved states when needed',
    option_d: 'Use sliding window compression to maintain only the most recent interactions',
    correct_answer: 'B',
  ...
  // Question 19 - Answer: A
  {
    ...
    option_b: 'Omit the failed tool result and continue with successful tool responses',
    option_c: 'Send a stop_reason signal to restart the conversation from a clean state',
    option_d: 'Implement automatic retry with exponential backoff until the tool succeeds',
    correct_answer: 'A',
  ...
  // Question 20 - Answer: D
  {
    ...
    option_a: 'Claude detected a tool execution requirement and paused for processing',
    option_b: 'The response exceeded the configured token limit for the request',
    option_c: 'The agentic loop reached its maximum iteration count successfully',
  ...
  // Question 21 - Answer: C
  {
    ...
    option_a: 'basic, advanced, expert for progressive reasoning depth tiers',
    option_b: 'minimal, standard, maximum corresponding to token allocation levels',
    option_c: 'low, medium, high controlling the depth of reasoning analysis',
    option_d: 'shallow, balanced, deep determining the breadth versus depth tradeoff',
    correct_answer: 'C',
  ...
  // Question 22 - Answer: B
  {
    ...
    option_a: 'Return only the first tool result and queue the others for subsequent iterations',
    option_b: 'Return one tool_result for each tool_use, ensuring tool_use_id values match',
    option_c: 'Merge all three results into a single consolidated tool_result block',
    option_d: 'Return results in separate user messages to maintain execution isolation',
    correct_answer: 'B',
  ...
  // Question 23 - Answer: A
  {
    ...
    option_b: 'Stream raw binary data with progressive enhancement for optimal throughput',
    option_c: 'Use complete serialized HTML with embedded metadata for full fidelity',
    option_d: 'Compress data into base64-encoded archives to reduce token overhead',
    correct_answer: 'A',
  ...
  // Question 24 - Answer: D
  {
    ...
    option_a: 'Multiple tool requests are disabled by default for cost optimization',
    option_b: 'Only server-side tools support concurrent execution in single responses',
    option_c: 'Multiple tools require enabling the parallel_execution API flag',
  ...
  // Question 25 - Answer: C
  {
    ...
    option_a: 'Agentic loops reduce overall API costs through intelligent request batching',
    option_b: 'The implementation complexity is significantly lower with agentic patterns',
    option_c: 'Agentic loops enable iterative tool use and reasoning refinement for complex multi-step tasks',
    option_d: 'Single-turn responses provide faster time-to-completion for most workflows',
    correct_answer: 'C',
  ...
  // Question 26 - Answer: B
  {
    ...
    option_a: 'Store conversation as a flat text string with role delimiters',
    option_b: 'Build an array of message objects with alternating user/assistant roles and content arrays',
    option_c: 'Encode the full history as structured JSON within the system prompt',
    option_d: 'Maintain a binary-serialized buffer for optimal memory efficiency',
    correct_answer: 'B',
  ...
  // Question 27 - Answer: A
  {
    ...
    option_b: 'Validate response timestamps to ensure chronological ordering of events',
    option_c: 'Verify the payload size stays within the configured token budget limits',
    option_d: 'Confirm that all JSON structures conform to the schema specification',
    correct_answer: 'A',
  ...
  // Question 28 - Answer: D
  {
    ...
    option_a: 'Monitor aggregate token consumption across all workflows for cost tracking',
    option_b: 'Track error rates and failure patterns to identify systemic issues',
    option_c: 'Measure API response latency and throughput for performance optimization',
  ...
  // Question 29 - Answer: C
  {
    ...
    option_a: 'The API continues processing but charges overage fees to your account',
    option_b: 'You receive a budget warning notification but the loop continues executing',
    option_c: 'The loop terminates immediately with a budget exceeded error',
    option_d: 'The system automatically downgrades to a more cost-effective model tier',
    correct_answer: 'C',
  ...
  // Question 30 - Answer: B
  {
    ...
    option_a: 'tool_call content blocks containing executable function references',
    option_b: 'tool_use content blocks specifying tool name and parameters',
    option_c: 'tool_request content blocks defining the operation to perform',
    option_d: 'function_call content blocks with invocation details and arguments',
    correct_answer: 'B',
  ...
  // Question 31 - Answer: A
  {
    ...
    option_b: 'Providing excessive tool access without proper authorization scoping',
    option_c: 'Using overly complex system prompts that increased token consumption',
    option_d: 'Enabling streaming responses which bypassed normal rate limiting',
    correct_answer: 'A',
  ...
  // Question 32 - Answer: D
  {
    ...
    option_a: 'Silently omit the failed tool and continue with successful operations',
    option_b: 'Implement exponential backoff polling until the service becomes available',
    option_c: 'Automatically substitute with a fallback tool providing similar functionality',
  ...
  // Question 33 - Answer: C
  {
    ...
    option_a: 'No, the API restricts conversation history to the current session only',
    option_b: 'Historical context requires enabling the session_persistence configuration flag',
    option_c: 'Yes, you can include messages from previous sessions to maintain context across interactions',
    option_d: 'Only summarized versions of the most recent 5 messages are supported',
    correct_answer: 'C',
  ...
  // Question 34 - Answer: B
  {
    ...
    option_a: 'Continue the loop iteration to allow for additional processing cycles',
    option_b: 'Exit the loop - the task is complete or no tools are needed',
    option_c: 'Retry the identical request to confirm the completion status',
    option_d: 'Pause execution and wait for explicit user input to proceed',
    correct_answer: 'B',
  ...
  // Question 35 - Answer: A
  {
    ...
    option_b: 'Minimal context only - system prompts add unnecessary token overhead',
    option_c: 'Comprehensive tool definitions with complete parameter specifications',
    option_d: 'Historical conversation examples demonstrating successful review patterns',
    correct_answer: 'A',
  ...
  // Question 36 - Answer: D
  {
    ...
    option_a: 'Client-side tools provide identical execution characteristics with no tradeoffs',
    option_b: 'Server-side tools introduce additional network latency from remote execution',
    option_c: 'Server-side tools require higher per-request costs for infrastructure usage',
  ...
  // Question 37 - Answer: C
  {
    ...
    option_a: 'Set is_error: true with a standardized error code for categorization',
    option_b: 'Include structured error metadata with severity levels and retry hints',
    option_c: 'is_error: true plus a human-readable error description explaining what went wrong',
    option_d: 'Provide complete stack traces with debugging context for analysis',
    correct_answer: 'C',
  ...
  // Question 38 - Answer: B
  {
    ...
    option_a: 'Never enable it - extended thinking conflicts with tool-based workflows',
    option_b: 'For the complex reasoning tasks that require deeper analysis of legal precedents and regulations',
    option_c: 'Enable it universally for every iteration to maximize reasoning quality',
    option_d: 'Only when generating final summaries after all tool executions complete',
    correct_answer: 'B',
  ...
  // Question 39 - Answer: A
  {
    ...
    option_b: 'The API automatically infers relationships based on tool execution timestamps',
    option_c: 'Multiple tool_results can share the same tool_use_id for batched operations',
    option_d: 'Correspondence is established implicitly through sequential ordering in arrays',
    correct_answer: 'A',
  ...
  // Question 40 - Answer: D
  {
    ...
    option_a: 'Schema detail has minimal impact - Claude relies primarily on runtime behavior',
    option_b: 'Detailed schemas are required only for regulatory compliance documentation',
    option_c: 'Schemas affect execution performance through optimized caching strategies',
  ...
  // Question 41 - Answer: C
  {
    ...
    option_a: 'Claude intelligently proceeds by treating the missing result as null',
    option_b: 'The API assumes the missing tool executed successfully with empty output',
    option_c: 'The API returns a validation error requiring all tool_use blocks to have results',
    option_d: 'The system automatically retries with exponential backoff for missing results',
    correct_answer: 'C',
  ...
  // Question 42 - Answer: B
  {
    ...
    option_a: 'Each tool selection is independent - context provides no influence',
    option_b: 'Previous tool results and conversation history inform which tools Claude selects next',
    option_c: 'Only the single immediately previous turn influences decision-making',
    option_d: 'Tool selection uses randomized sampling weighted by past success rates',
    correct_answer: 'B',
  ...
  // Question 43 - Answer: A
  {
    ...
    option_a: 'Periodically summarize and compress earlier conversation turns strategically',
    option_b: 'Maintain complete uncompressed history for maximum context fidelity',
    option_c: 'Implement aggressive pruning by removing all messages older than 5 turns',
    option_d: 'Use statistical sampling to retain representative message subsets',
    correct_answer: 'A',
  ...
  // Question 44 - Answer: D
  {
    ...
    option_a: 'No, tool definitions become immutable after the first API call',
    option_b: 'Tool changes require terminating and restarting the entire conversation',
    option_c: 'Dynamic tool modification requires enterprise-tier API access privileges',
  ...
  // Question 45 - Answer: C
  {
    ...
    option_a: 'The system is performing optimally within configured constraints',
    option_b: 'Response times are excellent due to early termination mechanisms',
    option_c: 'The loop needs optimization - likely better tool design or improved prompts',
    option_d: 'Token consumption is efficiently controlled through iteration limits',
    correct_answer: 'C',
  ...
  // Question 46 - Answer: B
  {
    ...
    option_a: 'Silently omit the nonexistent tool and proceed with valid requests',
    option_b: 'Return an error tool_result explaining the tool does not exist',
    option_c: 'Dynamically generate a placeholder tool stub to satisfy the request',
    option_d: 'Terminate the workflow immediately to prevent cascading errors',
    correct_answer: 'B',
  ...
  // Question 47 - Answer: A
  {
    ...
    option_b: 'This is mandatory metadata required by the API protocol specification',
    option_c: 'This overhead consumes tokens unnecessarily and should be suppressed',
    option_d: 'This only appears when verbose logging or debugging mode is active',
    correct_answer: 'A',
  ...
  // Question 48 - Answer: D
  {
    ...
    option_a: 'Yes, results must be alphabetically sorted by tool name for validation',
    option_b: 'Yes, maintaining strict execution order prevents correlation errors',
    option_c: 'Yes, reverse ordering is required for proper stack unwinding',
  ...
  // Question 49 - Answer: C
  {
    ...
    option_a: 'Description verbosity has no measurable impact on tool selection accuracy',
    option_b: 'Excessive detail increases confusion and reduces selection precision',
    option_c: 'Clear, detailed descriptions help Claude select and use tools appropriately',
    option_d: 'Descriptions only influence error message quality, not execution behavior',
    correct_answer: 'C',
  ...
  // Question 50 - Answer: B
  {
    ...
    option_a: 'Always execute to max_turns limit to ensure thorough task completion',
    option_b: 'Exit early when stop_reason is end_turn, max_tokens, or success conditions are met',
    option_c: 'Terminate after the first successful tool execution to optimize costs',
    option_d: 'Use probabilistic early stopping based on confidence thresholds',
    correct_answer: 'B',
  ...
  // Question 51 - Answer: A
  {
    ...
    option_b: 'Expand tool inventory significantly to provide more selection options',
    option_c: 'Minimize descriptions to reduce token overhead and simplify decisions',
    option_d: 'Implement probabilistic tool selection with weighted randomization',
    correct_answer: 'A',
  ...
  // Question 52 - Answer: D
  {
    ...
    option_a: 'Apply base64 encoding to compress payload size for efficient transmission',
    option_b: 'Stream raw binary data directly to minimize processing overhead',
    option_c: 'Transmit complete unfiltered datasets to preserve information fidelity',
  ...
  // Question 53 - Answer: C
  {
    ...
    option_a: 'No, the API restricts parameters to primitive scalar types only',
    option_b: 'Only homogeneous arrays are supported, not nested object structures',
    option_c: 'Yes, tools can receive complex nested JSON parameters with objects and arrays',
    option_d: 'Parameters must be flattened into key-value string pairs for transmission',
    correct_answer: 'C',
  ...
  // Question 54 - Answer: B
  {
    ...
    option_a: 'Only the final response output that Claude generated before failure',
    option_b: 'Complete conversation history, stop_reasons for each turn, all tool calls, and their results',
    option_c: 'Exclusively the error messages and stack traces from failed tools',
    option_d: 'Aggregate token consumption and latency statistics for the session',
    correct_answer: 'B',
  ...
  // Question 55 - Answer: A
  {
    ...
    option_b: 'Prompt caching conflicts with tool use and cannot be enabled together',
    option_c: 'It stores only final execution results to accelerate repeated queries',
    option_d: 'Caching is automatically disabled when agentic loops are detected',
    correct_answer: 'A',
  ...
  // Question 56 - Answer: D
  {
    ...
    option_a: 'Begin with a tool_use block to initialize the execution context',
    option_b: 'Start with an empty conversation array and build incrementally',
    option_c: 'Initialize with an assistant message containing default instructions',
  ...
  // Question 57 - Answer: C
  {
    ...
    option_a: 'Always execute synchronously to ensure deterministic result ordering',
    option_b: 'Always execute asynchronously to maximize throughput and minimize latency',
    option_c: 'It depends on the use case - independent tools can run in parallel, dependent ones sequentially',
    option_d: 'Asynchronous execution is not supported in the current API version',
    correct_answer: 'C',
  ...
  // Question 58 - Answer: B
  {
    ...
    option_a: 'Dependent tool chains are not supported in agentic patterns',
    option_b: 'Execute them sequentially across loop iterations - return first result to Claude, let it request the second tool',
    option_c: 'Combine both operations into a single composite tool definition',
    option_d: 'Claude automatically detects dependencies and executes tools in correct order',
    correct_answer: 'B',
  ...
  // Question 59 - Answer: A
  {
    ...
    option_b: 'Tool selection follows stochastic sampling and cannot be deterministically influenced',
    option_c: 'System prompts override tool use entirely and disable all tool access',
    option_d: 'System prompts only control error handling behavior, not tool selection logic',
    correct_answer: 'A',
  ...
  // Question 60 - Answer: D
  {
    ...
    option_a: 'No, enabling streaming automatically disables all tool functionality',
    option_b: 'Streaming only works for pure text generation without tool execution',
    option_c: 'Streaming requires development environment and is disabled in production',
  ...
  // Question 61 - Answer: C
  {
    ...
    option_a: 'The parameters are functionally redundant and one should be removed',
    option_b: 'Both are mandatory requirements specified by the API documentation',
    option_c: 'Together they provide defense-in-depth: max_turns prevents excessive iterations, max_budget_usd caps spending',
    option_d: 'Setting both parameters has no measurable benefit over using either alone',
    correct_answer: 'C',
  ...
  // Question 62 - Answer: B
  {
    ...
    option_a: 'Always conceal the execution process and display only final results',
    option_b: 'Show intermediate tool calls and reasoning to provide transparency and build trust',
    option_c: 'Present only the polished final output without exposing workflow details',
    option_d: 'Display only error states while hiding successful operation details',
    correct_answer: 'B',
  ...
  // Question 63 - Answer: A
  {
    ...
    option_b: 'Avoid timeouts completely to allow tools unlimited execution time',
    option_c: 'Configure indefinite waiting to guarantee complete data retrieval',
    option_d: 'Enable automatic infinite retry loops until successful completion',
    correct_answer: 'A',
  ...
  // Question 64 - Answer: D
  {
    ...
    option_a: 'No, extended thinking and tool use are mutually exclusive features',
    option_b: 'Compatibility exists only in development mode, not production',
    option_c: 'Only enterprise-tier models support this feature combination',
  ...
  // Question 65 - Answer: C
  {
    ...
    option_a: 'The API automatically truncates oversized results to fit context limits',
    option_b: 'Results undergo automatic compression before transmission to the API',
    option_c: 'The request fails due to context overflow - you need to manage result size by summarizing',
    option_d: 'The system automatically prunes older messages to accommodate new data',
    correct_answer: 'C',
  ...
  // Question 66 - Answer: B
  {
    ...
    option_a: 'Completely remove the problematic tool from available options',
    option_b: 'Use system prompt guidance and tool descriptions to set expectations about when to use this tool',
    option_c: 'Tool usage frequency is autonomous and cannot be influenced',
    option_d: 'Implement probabilistic rejection to randomly block excessive calls',
    correct_answer: 'B',
  ...
  // Question 67 - Answer: A
  {
    ...
    option_b: 'These represent completely independent concepts with no relationship',
    option_c: 'Workflow orchestration supersedes and replaces agentic loop patterns',
    option_d: 'The approaches are incompatible and cannot coexist in one system',
    correct_answer: 'A',
  ...
  // Question 68 - Answer: D
  {
    ...
    option_a: 'No, Claude generates parameters that are always correctly formatted',
    option_b: 'Validation is only necessary during development, not in production',
    option_c: 'Only validate parameters when calling external third-party APIs',
  ...
  // Question 69 - Answer: C
  {
    ...
    option_a: 'Result formatting has negligible impact on Claude\'s reasoning capabilities',
    option_b: 'Formatting primarily affects execution speed, not decision quality',
    option_c: 'Well-structured, clear results significantly improve Claude\'s ability to understand and reason',
    option_d: 'Only strict JSON format is supported for proper parsing',
    correct_answer: 'C',
  ...
  // Question 70 - Answer: B
  {
    ...
    option_a: 'Continue retrying indefinitely with exponential backoff until success',
    option_b: 'Return descriptive error messages via tool_result and let Claude adapt or suggest alternatives',
    option_c: 'Terminate the application immediately to alert operations team',
    option_d: 'Suppress all error signals and continue silently without notification',
    correct_answer: 'B',
  ...
  // Question 71 - Answer: A
  {
    ...
    option_b: 'Disable logging in production to optimize performance and reduce costs',
    option_c: 'Log exclusively error events and failure conditions for troubleshooting',
    option_d: 'Record only successful workflow completions to track productivity',
    correct_answer: 'A',
  ...
  // Question 72 - Answer: D
  {
    ...
    option_a: 'max_turns must remain constant across all workflow types',
    option_b: 'The parameter can only be decreased, never increased during execution',
    option_c: 'Dynamic adjustment is not supported by the current API',
  ...
  // Question 73 - Answer: C
  {
    ...
    option_a: 'Maximize tool count to provide comprehensive coverage of all operations',
    option_b: 'Build generic multi-purpose tools to minimize implementation complexity',
    option_c: 'Design clear, focused tools that solve specific problems with excellent descriptions',
    option_d: 'Implement sophisticated tools with extensive parameter specifications',
    correct_answer: 'C',
  ...
  // Question 74 - Answer: B
  {
    ...
    option_a: 'Enforce the specific sequence that performs best in benchmarks',
    option_b: 'Let Claude choose based on context and its reasoning',
    option_c: 'Always select whichever approach has the fewest steps',
    option_d: 'Implement randomized selection to ensure balanced usage',
    correct_answer: 'B',
  ...
  // Question 75 - Answer: A
  {
    ...
    option_b: 'Deploy directly to production and test with live user traffic',
    option_c: 'Skip testing since Claude provides reliable autonomous operation',
    option_d: 'Focus exclusively on testing the primary happy path workflow',
    correct_answer: 'A',
  ...
  // Question 76 - Answer: D
  {
    ...
    option_a: 'No, the API requires maintaining complete conversation history indefinitely',
    option_b: 'You can only prune user messages while keeping all assistant responses',
    option_c: 'Deletion is limited to tool_result blocks, not conversational messages',
  ...
  // Question 77 - Answer: C
  {
    ...
    option_a: 'They are functionally identical and can be used interchangeably',
    option_b: 'pause_turn signals that a critical error occurred during execution',
    option_c: 'end_turn means Claude finished naturally, pause_turn means Claude wants explicit user approval',
    option_d: 'end_turn always indicates pending tool execution requirements',
    correct_answer: 'C',
  ...
  // Question 78 - Answer: B
  {
    ...
    option_a: 'No statistically significant improvement over single-turn approaches',
    option_b: '50%+ improvement in complex task success rates compared to baseline',
    option_c: 'Modest 5-10% improvement with high variance across tasks',
    option_d: 'Performance degradation due to increased overhead and latency',
    correct_answer: 'B',
  ...
  // Question 79 - Answer: A
  {
    ...
    option_b: 'Providing an extensive library with maximum tool variety and options',
    option_c: 'Prioritizing execution speed optimization above all other considerations',
    option_d: 'Selecting the largest available model regardless of task requirements',
    correct_answer: 'A',
  ...
  // Question 80 - Answer: D
  {
    ...
    option_a: 'They provide equivalent functionality through different naming conventions',
    option_b: 'max_turns has been deprecated in favor of max_budget_usd',
    option_c: 'max_budget_usd requires enterprise-tier subscription access',
  ...
  // Question 81 - Answer: C
  {
    ...
    option_a: 'Implement infinite retry loops until eventual successful execution',
    option_b: 'Fail immediately on first error to ensure rapid failure detection',
    option_c: 'Implement exponential backoff with maximum retry limits to handle transients',
    option_d: 'Apply retry logic only for server-side tools, not client-side',
    correct_answer: 'C',
  ...
  // Question 82 - Answer: B
  {
    ...
    option_a: 'System prompts are not compatible with agentic workflow patterns',
    option_b: 'System prompts are provided as a separate API parameter, not in the message history',
    option_c: 'System prompts must appear as the first element in the messages array',
    option_d: 'System prompts function as replacement for initial user messages',
    correct_answer: 'B',
  ...
  // Question 83 - Answer: A
  {
    ...
    option_b: 'Return complete unfiltered data to maintain information completeness',
    option_c: 'Apply end-to-end encryption to all tool results before transmission',
    option_d: 'Prohibit any tools from accessing databases containing sensitive data',
    correct_answer: 'A',
  ...
  // Question 84 - Answer: D
  {
    ...
    option_a: 'Never check pause_turn as it has been deprecated in recent versions',
    option_b: 'Check on every single iteration regardless of the operation type',
    option_c: 'Only monitor pause_turn during development and debugging phases',
  ...
  // Question 85 - Answer: C
  {
    ...
    option_a: 'Minimal function signatures with abbreviated parameter names only',
    option_b: 'Exhaustive documentation with every possible implementation detail',
    option_c: 'Clear descriptions, well-defined parameter types, constraints, and usage examples',
    option_d: 'Standard code-level comments embedded in the implementation',
    correct_answer: 'C',
  ...
  // Question 86 - Answer: B
  {
    ...
    option_a: 'The loop continues unaffected using cached responses automatically',
    option_b: 'The API returns a rate limit error that your code should handle gracefully with retry logic',
    option_c: 'The system implements infinite automatic retries with exponential backoff',
    option_d: 'The loop enters a mandatory 24-hour cooling-off period',
    correct_answer: 'B',
  ...
  // Question 87 - Answer: A
  {
    ...
    option_b: 'No, each tool must consistently use a single predefined format',
    option_c: 'Only binary-encoded data formats are supported by the API',
    option_d: 'Only unstructured plain text responses are allowed',
    correct_answer: 'A',
  ...
  // Question 88 - Answer: D
  {
    ...
    option_a: 'This parameter is not available in agentic workflow patterns',
    option_b: 'It only functions during debugging and development environments',
    option_c: 'This is a legacy deprecated parameter from earlier API versions',
  ...
  // Question 89 - Answer: C
  {
    ...
    option_a: 'Return only the successful records and omit failure information',
    option_b: 'Mark the entire batch operation as failed with is_error: true',
    option_c: 'Return a comprehensive result detailing which operations succeeded and which failed with reasons',
    option_d: 'Automatically retry all 10 records to ensure complete success',
    correct_answer: 'C',
  ...
  // Question 90 - Answer: B
  {
    ...
    option_a: 'Completely disable tool functionality to eliminate confusion risks',
    option_b: 'Provide accurate tool schemas with clear descriptions and validate all tool_use parameters',
    option_c: 'Use only generic ambiguous tool names to reduce specificity',
    option_d: 'Remove all tool descriptions to minimize misleading information',
    correct_answer: 'B',
  ...
  // Question 91 - Answer: A
  {
    ...
    option_b: 'Avoid logging metrics entirely as it introduces excessive overhead',
    option_c: 'Capture metrics only when error conditions or failures occur',
    option_d: 'Record metrics only at workflow initialization and completion',
    correct_answer: 'A',
  ...
  // Question 92 - Answer: D
  {
    ...
    option_a: 'Server-side tools provide lower per-execution cost structures',
    option_b: 'Server-side tools require more complex implementation patterns',
    option_c: 'Server-side tools have no meaningful advantages over client-side',
  ...
  // Question 93 - Answer: C
  {
    ...
    option_a: 'Skip formal documentation - well-written code is self-documenting',
    option_b: 'Add inline code comments throughout the implementation',
    option_c: 'Document the loop logic, tool schemas, error handling patterns, and monitoring approach',
    option_d: 'Create a basic README file with installation instructions',
    correct_answer: 'C',
  ...
  // Question 94 - Answer: B
  {
    ...
    option_a: 'Individual tool latency has minimal impact on total workflow runtime',
    option_b: 'High latency compounds across iterations, significantly impacting total runtime',
    option_c: 'Latency primarily affects API billing costs, not end-user experience',
    option_d: 'The system applies automatic latency optimization algorithms',
    correct_answer: 'B',
  ...
  // Question 95 - Answer: A
  {
    ...
    option_b: 'No, the model selection must remain constant throughout workflows',
    option_c: 'Model switching requires special enterprise-tier API permissions',
    option_d: 'The system automatically switches models based on task complexity',
    correct_answer: 'A',
  ...
  // Question 96 - Answer: D
  {
    ...
    option_a: 'Security is inherently guaranteed by the Anthropic API infrastructure',
    option_b: 'Focus logging exclusively on successful operations for audit trails',
    option_c: 'Disable all external tool integrations to eliminate attack vectors',
  ...
  // Question 97 - Answer: C
  {
    ...
    option_a: 'Probabilistic random selection after executing a minimum number of iterations',
    option_b: 'Claude systematically executes all available tools before terminating',
    option_c: 'Claude reasons about whether it has gathered sufficient information to complete the task',
    option_d: 'Only explicit timeout-based limits determine when workflows complete',
    correct_answer: 'C',
  ...
  // Question 98 - Answer: B
  {
    ...
    option_a: 'Always execute all tools sequentially to avoid race conditions',
    option_b: 'Execute the two independent database queries in parallel, then run the dependent API call sequentially',
    option_c: 'Use strictly sequential execution to ensure deterministic ordering',
    option_d: 'Randomize execution order to balance load distribution',
    correct_answer: 'B',
  ...
  // Question 99 - Answer: A
  {
    ...
    option_a: 'The API returns a validation error for mismatched identifiers',
    option_b: 'Claude silently ignores and discards the mismatched results',
    option_c: 'Results are automatically matched by sequential array position',
    option_d: 'Claude uses heuristics to infer which results correspond to which tools',
    correct_answer: 'A',
  ...
  // Question 100 - Answer: D
  {
    ...
    option_a: 'No measurable improvement compared to single-turn API approaches',
    option_b: 'Performance degradation due to increased complexity overhead',
    option_c: 'Marginal 5-10% improvement with high variance across tasks',
  ...
  // Question 101 - Answer: C
  {
    ...
    option_a: 'Agree completely - fewer iterations always indicates superior efficiency',
    option_b: 'Iteration count is completely irrelevant to measuring system quality',
    option_c: 'Multiple iterations enable thorough problem-solving; what matters is task success rate, not minimizing iterations',
    option_d: 'All workflows should converge to identical iteration counts',
    correct_answer: 'C',
